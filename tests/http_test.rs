...
async fn test_http_routes() {
    ...
    // list of routes
    let router = init_app_a(global_config.clone(), context);
...
pub fn init_app_a(global_config: Config, cx: Context) -> Router<Context> {
    let mut router = Router::new().with_state(cx);
